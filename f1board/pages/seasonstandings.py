...
# Points Historic
historic_points = df_2019_2.groupby('driverRef').agg({'points': 'sum'}).sort_values('points',
...
top2019 = top2019.groupby(['surname'])[['points', 'wins']].max().sort_values('points', ascending=False).head(
    20).reset_index()
...
top2020 = top2020.groupby(['surname'])[['points', 'wins']].max().sort_values('points', ascending=False).head(
    20).reset_index()
...
top2021 = top2021.groupby(['surname'])[['points', 'wins']].max().sort_values('points', ascending=False).head(
    20).reset_index()
...
top2022 = top2022.groupby(['surname'])[['points', 'wins']].max().sort_values('points', ascending=False).head(
    20).reset_index()
...
layout = html.Div([
    ...
    html.Div(className='service-box', children=[
        html.Div(className='row',
                 children=[html.P("Choose the Season: ", style={'font-size': '20px', 'font-weight': 'bold'}),
        ...
        dcc.Graph(id='season', figure=fig2019),
    ])
    
])


@callback(Output('season', 'figure'), Input('dropdown', 'value'))
